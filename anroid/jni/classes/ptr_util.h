...
static void argValue(const v8::FunctionCallbackInfo<Value> &info, int index,
		float* slot) {
 ...
 */
template<typename T>
static void flatVector(const v8::FunctionCallbackInfo<Value> &info, T* values,
		int length) {
	int copyed = 0;
	...
	for (int i = 0; i < alen; i++) {
		...
		if (info[i]->IsArray()) {
			...
			int arrayPop = populateValues<T>(values + copyed, array,
					length - copyed);
			copyed += arrayPop;
			continue;
		}
		...
		if (p == 0) {
			...
			if (classtype::isTheSameType(vType, fPtr.mElement)) {
				...
				populateValues(values + copyed, fPtr.value_ptr(), fPtr.mElement,
						maxCp);
			}
			copyed += maxCp;
		}
 ...
 */
static void _valueFn(const v8::FunctionCallbackInfo<Value>& args, int eUnit,
		char* mPtr, int eSize) {
	while (1) {
		if (args.Length() == 0) {
			break;
		...
		if (type == CLASS_ArrayBuffer) {
			...
			args.GetReturnValue().Set(
					ThrowException(
							String::New(
									"_value args[0] cannot be treated as a buffer obejct.")));
		}
		break;
	}

	Handle<Object> byteArray = ClassWrap<NodeBuffer>::newInstance(
			Integer::NewFromUnsigned(eSize * eUnit));
	NodeBuffer* bPtr = internalPtr<NodeBuffer>(byteArray);
	bPtr->_writeDatas(0, eUnit, mPtr, eSize);
	args.GetReturnValue().Set(
			ClassWrap<TypedBuffer<float> >::newInstance(byteArray));
}
